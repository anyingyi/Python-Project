...
ACTIVATION = nn.ReLU()
LR = 0.03
B_INIT = -0.2  # use a band bias constant initializer
...
class Net(nn.Module):
    def __init__(self, batch_normalization=False):
        super(Net, self).__init__()  # must add,
        self.do_bn = batch_normalization
        self.fcs = []
        self.bns = []
        self.bn_input = nn.BatchNorm1d(1, momentum=0.5)  # momentum:weight for parameter(gamma & beta) in batch
        # normalization formula

        # set hidden layers by for loop
        for i in range(N_HIDDEN):
            input_size = 1 if i == 0 else 10
            fc = nn.Linear(input_size, 10)  # use self.fc=fc to add it to layer,can be writen as follow one
...
if __name__ == "__main__":
    ...
    for epoch in range(EPOCH):
        ...
        for net, l in zip(nets, losses):
            ...
            layer_input.append(layer_input)
    ...
    # evaluation
    [net.eval() for net in nets]  # set to eval mode(disable dropout and batch normalization)
