...
N_TEST_IMG = 5  # number of image to show as demo
...
class AutoEncoder(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(128, 28 * 28),
            # the pixel value of origin is (0,1), so the decoder should be same range, use sigmoid function
            nn.Sigmoid(),
        )
...
for epoch in range(EPOCH):
    for step, (b_x, b_y) in enumerate(train_loader):
        b_x = b_x.view(-1, 28 * 28)  # batch x,shape (batch,28*28)
